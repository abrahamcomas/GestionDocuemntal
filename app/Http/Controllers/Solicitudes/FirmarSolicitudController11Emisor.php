...
class FirmarSolicitudController11Emisor extends Controller
{
    public function index(Request $request)   
    {
        ...
        { 
            ...
            $datos=DB::table('DestinoDocumento11')->Select('Ruta_T')->where('Ruta_T', '=', $Ruta)->first();
            ...
            if(empty($responseBody->status)){ 
                ...
                if($responseFiles = $responseBody->files!=null){ 
                    ...
                    if($status=='OK'){
                        ...
                        $DestinoDocumento  =  DB::table('DestinoDocumento11')->where('Ruta_T', '=',$Ruta)->first();
                
                        $ID_DestinoDocumento   = $DestinoDocumento->ID_DestinoDocumento;
                        $DOC_ID_Documento    = $DestinoDocumento->DOC_ID_Documento;
                                        
                        $ID_DocumentoFirma  =  DB::table('DocumentoFirma11') 
                        ...
                        $DocumentoFirma             =DocumentoFirma11::find($ID_DocumentoF);
                        $DocumentoFirma->FechaFirma = date("Y/m/d");
                        $DocumentoFirma->Firmado    = 1;
                        $DocumentoFirma->save();

                        $Portafolio              = Portafolio11::find($DOC_ID_Documento);
        ...
        return view('Solicitudes/Resultado', compact('status'));
    }
}
