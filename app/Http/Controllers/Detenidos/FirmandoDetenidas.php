...
namespace App\Http\Controllers\Detenidos;
...
class FirmandoDetenidas extends Controller
{
    public function index(Request $request)   
    {
        ...
        { 
            ...
            if(empty($responseBody->status))
                        ...
                        if($status=='OK')
                            {
                                ...
                                $responseFiles = $responseBody->files;
                                    ...
                                    ->where('Firmado', '=',0)
                                    ->where('ID_Documento_T', '=',  $ID_Documento_T)->count(); 

                                  
                                    if($Cantidad==0){

                                        $Creado =  DB::table('Portafolio') 
                                        ->select('ODP') 
                ...
                return view('Documentos/DocumentoFirmadoDetenidos', compact('status'));
        }                                
        else
        {
            return back()
